...
class Commission(models.Model):
    ...
    name = fields.Char(string='Commission Reference', required=True, tracking=True, default='New', readonly=True)
    manufacturing_order_id = fields.Many2one('idil.manufacturing.order', string='Manufacturing Order', required=True,
                                             readonly=True)
    employee_id = fields.Many2one('idil.employee', string='Employee', required=True, readonly=True)
    commission_amount = fields.Float(string='Commission Amount', digits=(16, 5), required=True, readonly=True)
    commission_paid = fields.Float(string='Commission Paid', compute='_compute_commission_paid', store=True,
                                   readonly=True)
    commission_remaining = fields.Float(string='Commission Remaining', compute='_compute_commission_remaining',
    ...
    amount = fields.Float(string='Amount to Pay', digits=(16, 5), default=0.0, required=True)
    ...
    commission_payment_ids = fields.One2many('idil.commission.payment', 'commission_id', string='Commission Payments',
                                             readonly=True)
    ...
    def pay_commission(self):
        ...
        if not self.cash_account_id:
            raise ValidationError("Please select a cash account before paying the commission.")
    ...
    def _compute_commission_paid(self):
        for record in self:
            record.commission_paid = sum(payment.amount for payment in record.commission_payment_ids)
...
class CommissionPayment(models.Model):
    ...
    commission_id = fields.Many2one('idil.commission', string='Commission', required=True)
    ...
    booking_line_ids = fields.One2many('idil.transaction_bookingline', 'commission_payment_id', string='Booking Lines')
