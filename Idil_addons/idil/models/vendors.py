...
class Vendor(models.Model):
    ...
    def _check_phone(self):
        for record in self:
            if not record.phone.isdigit() or len(record.phone) < 10:
                raise ValidationError("Phone number must be at least 10 digits and contain only numbers.")
    ...
    def create(self, vals):
        # Custom logic before creating a vendor
        return super(Vendor, self).create(vals)

    def write(self, vals):
        # Custom logic before writing to a vendor
        return super(Vendor, self).write(vals)
...
class VendorBalanceReport(models.TransientModel):
    ...
    balance = fields.Float(string="Balance", store=True)  # Assuming you want to store and display this field
    ...
    def generate_vendor_balances_report(self):
        ...
        for balance in account_balances:
            self.create({
                ...
                'balance': balance['balance'],  # Make sure to store the calculated balance here
    ...
    def _get_vendor_balances(self):
        vendor_balances = []
        vendor_personnel = self.env['idil.vendor.registration'].search([('active', '=', True)])
        for vendor in vendor_personnel:
            # Initialize balance for each salesperson.
            booking_lines_balance = 0
            purchase_orders = self.env['idil.purchase_order'].search([('vendor_id', '=', vendor.id)])
            for order in purchase_orders:
                bookings = self.env['idil.transaction_booking'].search([('order_number', '=', order.id)])
            ...
            _logger.debug(f"Vendor Person: {vendor.name}, Balance: {booking_lines_balance}")

            vendor_balances.append({
                'vendor_id': vendor.id,
                'vendor_name': vendor.name,
                'vendor_tel': vendor.phone,
                'account_id': vendor.account_payable_id.id if vendor.account_payable_id else '',
                'account_name': vendor.account_payable_id.name if vendor.account_payable_id else False,
                'account_code': vendor.account_payable_id.code if vendor.account_payable_id else '',
...
class VendorTransactionReport(models.TransientModel):
    ...
    def generate_vendor_transaction_report(self):
        ...
        for vendor in vendors:
            ...
            account_ids = [vendor.account_payable_id.id, vendor.account_receivable_id.id]
            ...
            for transaction in transactions:
                ...
                self.create({
                    ...
                    'debit': transaction.dr_amount if transaction.transaction_type == 'dr' else 0,
                    'credit': transaction.cr_amount if transaction.transaction_type == 'cr' else 0,
