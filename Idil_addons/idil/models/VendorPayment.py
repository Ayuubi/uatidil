...
class VendorPayment(models.Model):
    ...
    vendor_transaction_id = fields.Many2one('idil.vendor_transaction', string='Vendor Transaction', ondelete='cascade')
    ...
    #                 'paid_amount': vendor_transaction.paid_amount + amount_difference,
    #                 'remaining_amount': vendor_transaction.amount - (vendor_transaction.paid_amount + amount_difference)
    #             })
    def _update_related_booking_and_transaction(self, amount_difference):
        transaction_booking = self.vendor_transaction_id.transaction_booking_id
        if transaction_booking:
            updated_paid_amount = transaction_booking.amount_paid + amount_difference
            remaining_amount = transaction_booking.amount - updated_paid_amount
            payment_status = 'partial_paid' if 0 < updated_paid_amount < transaction_booking.amount else (
                'paid' if remaining_amount == 0 else 'pending')
            transaction_booking.write({
                'amount_paid': updated_paid_amount,
                'remaining_amount': remaining_amount,
            ...
            if vendor_transaction:
                ...
                vendor_payment_status = 'partial_paid' if 0 < vendor_paid_amount < vendor_transaction.amount else (
                    'paid' if vendor_remaining_amount == 0 else 'pending')
                vendor_transaction.write({
